...
def FetchJudgeName(file_text):
    ...
    print("Fetching JudgeName and Judge...")
    result = []
    result.append("\n")
    doc = nlp(file_text)
    sentences = list(doc.sents)

    #Breaking the File into chunks and giving second chunk to fetch Court Name and Name of the Judge
    #1 Chunk size is 1000 characters
    ...
    texts = custom_text_splitter.create_documents([file_text])
    
    if len(texts) < 2:
        result.append("Insufficient text chunks to process.")
        return result
    
    #This chunk has required info
    chunk_stored = texts[1].page_content
    ...
    questions = {
        "Determining Judge:": "Who is the Determining Judge?"
    }
    ...
    for key, question in questions.items():
        try:
            answer = qa_pipeline(question=question, context=chunk_stored)
            results[key] = answer['answer']
        except Exception as e:
            results[key] = str(e)

    for key, result_text in results.items():
        result.append(f"{key} {result_text}\n")

    print("Ending FetchJudgeName Function")
    return result
...
def ClaimantPosition(FileText):
    ...
    entities = [(ent.text, ent.start_char, ent.end_char, ent.label_) for ent in doc.ents if "Appellant" or "solicitors" or "Claimant" in ent.text and ent.label_ in {"ORG", "PERSON"}]
...
def DefendantPosition(FileText):
    ...
    result1 = ["Defendant Argues in the Court:\n\n"]
    ...
    #Using NLP and filter searching to find Appelant word or entity occured in the File
    doc = nlp(FileText)
    entities = [(ent.text, ent.start_char, ent.end_char, ent.label_) for ent in doc.ents if "Defendant" or "Respondent" or "defedant's" or "defedant" in ent.text and ent.label_ in {"ORG", "PERSON"}]
...
def main():
    directory = r"C:\Users\DELL\OneDrive\Desktop\FINAL\BailiArticles"
    output_directory = r"C:\Users\DELL\OneDrive\Desktop\FINAL\Bailisummaryoutput"
    ...
    for FileText, FileName in files:
        ...
        output_content.extend(FetchCourtName(FileName))
